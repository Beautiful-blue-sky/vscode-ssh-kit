...
- 最近连接主机自动显示在列表顶部。
- 按主机名、地址或标签搜索并连接。
- 可直接筛选主机树，支持 Host 昵称、IP / HostName、用户、端口、分组和标签；多个空格分隔的关键词会组合匹配，旁边的清除按钮可恢复完整列表。
...
- 连通性测试使用 OpenSSH `StrictHostKeyChecking=accept-new`：首次出现的主机指纹可自动接受，但指纹发生变化时仍会阻止连接。
...
- **SSH Kit 数据备份：** **备份数据** 命令会先询问仅导出主机数据，还是同时包含关联密钥。完整备份包含私钥内容，请保存到加密或访问受控的位置，迁移完成后及时删除临时备份。

运行时提示和树视图文本支持英文及简体中文。持久化数据带有 schema 版本，加载旧版数据时会校验并迁移；格式异常的备份记录会在恢复前被拒绝。
