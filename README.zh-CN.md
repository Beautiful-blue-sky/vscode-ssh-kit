...
- VS Code 窗口重新获得焦点时，会从共享 Catalog 刷新主机和密钥视图，其他窗口中的修改无需手动刷新即可显示。
- 编辑、删除、复制主机名、测试或连接前会按 ID 重新读取 Catalog 中的最新记录，其他 VS Code 窗口里尚未刷新的旧树节点不会继续使用过期主机数据。
- 每台主机保存唯一的 Remote-SSH Host alias。修改显示名称会原位重命名托管 Host alias，修改地址、端口、用户或认证方式时则保持 alias 稳定；两种操作都不会创建第二条主机记录。
- 升级后，SSH Kit 会检测旧版本修改昵称时遗留的 Host alias，但不会静默改写。确认窗口会逐项展示旧值和建议值，先创建内部快照，再统一更新 Catalog 与托管配置；本次暂不处理时，仍可从“管理 Remote-SSH 连接配置”重新进入。
...
- 日常新增、编辑、删除和恢复主机只重建托管文件；生成内容没有变化时不会重复写盘。用户原有 Host、注释、`Include` / `Match`、全局指令或其他工具管理的内容不会被改写。
- **导出主机为 SSH Config** 只写入用户选择的独立文件，拒绝替换当前生效的 SSH Config。
- **管理 Remote-SSH 连接配置** 会显示当前状态，并只列出当下可用的准备、修复、打开配置、旧昵称 alias 检查、移除和旧版连接别名清理操作。任何会修改生效 SSH Config 的操作都必须先由用户选择本次备份文件。
- 只有实际存在旧版连接别名时才会提供清理入口；升级不会静默删除它们。
...
- 删除分组、将主机移入或移出回收站、永久删除或清空回收站、恢复 JSON 数据、修复旧昵称遗留 alias，以及恢复另一份快照时，都会创建不含私钥的内部快照。SSH Kit 保留最近 10 份，可通过主机视图菜单或命令面板恢复。
...
- 会把恢复主机的密钥路径改写为本机实际写入、改名或复用的密钥路径；跳过或恢复失败的密钥会清空关联及残留的仅密钥认证指令，不保留源机器设置。
...
| `SSH Kit: 管理 Remote-SSH 连接配置` | 显示当前状态，并只提供当下适用的准备、修复、旧昵称别名检查、移除、配置文件和旧版连接别名清理操作 |
...
- **内部 Catalog 快照：** 危险目录操作和用户确认的旧昵称 alias 修复会创建快照，最多保留最近 10 份，不包含私钥。
