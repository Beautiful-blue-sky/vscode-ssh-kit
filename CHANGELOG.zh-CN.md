...
### 调整
- `sshKitHosts` 语言模型工具改为与主机树一致的多关键词匹配，支持分页，并会根据当前模型的 token 预算缩短返回结果。
- 明确 Agent 模式何时可能调用工具、工具可读取哪些元数据，以及 VS Code 批准设置如何影响主机清单共享。
- 本地化语言模型调用和密钥路径确认提示，并在确认中显示查询与分页范围。
- 主机输入支持 IPv6 和常见企业用户名；主机树、预览、状态详情和 AI 结果统一使用清晰的 IPv6 连接地址格式。
- 只扫描可识别的 OpenSSH 或 PEM 私钥；重命名或删除密钥时同步维护主机的认证文件关联。
- SSH Config 导入、写回和 Remote-SSH 连接别名共用同一套分词与格式化规则，支持含空格的带引号路径。
- 将存在安全问题的传递型开发依赖锁定到已修复版本。

### 修复
- 语言模型结果中的 IPv6 连接地址改用无歧义格式。
- SSH Config 中端口无效或超出范围时，导入结果回退到默认端口 22。
- 备份恢复前拒绝重复标识，以及包含换行、空字符或非法指令名称的 SSH Config 值。
- 避免孤立公钥文件以及部分失败的密钥恢复或重命名操作覆盖、破坏密钥对状态。
...
- 改用 Remote-SSH 原生命令参数传递 Host alias，让 `web+cache+gateway` 这类名称不再出现 `%2B` 或被 `+` 截断。
